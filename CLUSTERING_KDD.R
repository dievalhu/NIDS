...
on = which(nids1$V42 %in% c(1))
m1 = sample(on,(round(le*0.569))) #proportion of ones in the test datased
ze = which(nids1$V42 %in% c(0))
m2 = sample(ze,(round(le*0.431))) #proportion of zeros in the test datased
m = c(m1,m2)
...
clase = as.factor(nids2$V42)
plot(x, y, col=c("red","blue")[clase], cex=.5, xlab="Dimension 1",ylab="Dimension 2")
legend(x = 2e+7, y = 6e+5, legend=c("Normal", "Anormal"), col=c("red", "blue"), lty = 1, cex=0.5, pch = 1, horiz=TRUE, box.lty=0, bty="n")
...
library(aricode)
AMI_e = AMI(nids2$V42+1,g1)
ARI_e = ARI(nids2$V42+1,g1)
library(cluster)
si = silhouette(g1,d)
...
AMI_e1 = AMI(nids2$V42+1,clus2)
ARI_e1 = ARI(nids2$V42+1,clus2)
library(cluster)
